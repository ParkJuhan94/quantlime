...
#
# 2026-08-15부로 텔레그램 요약은 글 1건이 아니라 "채널×날짜" 단위 다이제스트로
# 바뀌었다(하루 수십 건씩 올라오는 채널을 개별 요약하면 Gemini 쿼터 소모 대비
# 정보 밀도가 낮다는 판단) - transcript_content 자리에 그날 게시글 여러 개를
# 구분선(---)으로 이어붙인 텍스트가 들어오므로, 개별 나열이 아니라 하루치
# 흐름을 종합해 요약하라는 지시를 명시한다.
_PROMPT_INTRO_TELEGRAM = (
    "다음은 국내/해외 주식 투자 관련 텔레그램 채널에 하루 동안 올라온 여러 "
    "게시글을 구분선(---)으로 이어붙인 것입니다. 음성인식 자막이 아니라 "
    "작성자가 직접 작성한 글이므로 본문에 적힌 수치·고유명사는 그대로 신뢰해도 "
    "됩니다. 게시글을 하나씩 개별적으로 나열하지 말고, 하루 동안 있었던 일을 "
    "하나의 흐름으로 종합해 요약하세요. summary와 key_points, macro_points는 "
    "반드시 한국어로 답하세요. 오늘 있었던 일을 2~4문장으로 요약하세요.\n\n"
)
