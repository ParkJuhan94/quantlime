...
class DomesticDailyPriceServiceTest {
    ...
    void refreshRecent_missingRecentDays_savesOnlyNewOnes() {
        ...
        given(domesticDailyPriceRepository.existsByStockCodeAndTradeDate(eq(STOCK_CODE), eq(LocalDate.parse("2026-07-09"))))
            .willReturn(false);
        given(domesticDailyPriceRepository.existsByStockCodeAndTradeDate(eq(STOCK_CODE), eq(LocalDate.parse("2026-07-08"))))
            .willReturn(false);
        given(domesticDailyPriceRepository.existsByStockCodeAndTradeDate(eq(STOCK_CODE), eq(LocalDate.parse("2026-07-07"))))
            .willReturn(true);
