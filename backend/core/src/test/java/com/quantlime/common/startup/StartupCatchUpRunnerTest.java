...
class StartupCatchUpRunnerTest {
    ...
    // 엮일 수 있어(MarketDataRefreshServiceTest와 동일한 함정), 여기서는
    // 명시적으로 생성자를 호출한다.
    ...
    void setUp() {
        startupCatchUpRunner = new StartupCatchUpRunner(
            ...
            marketDataCatchUpTaskExecutor, videoFeedCatchUpTaskExecutor);
    }

    @Test
    @DisplayName("[기동 시 가격/스코어 갭필은 전용 실행기에, 영상 피드 캐치업(수집→자막→요약→보존기간 정리)은 별도 전용 실행기에 각각 제출한다]")
