...
@Tag("unit")
class DomesticMarketRankingCacheTest {

    private final DomesticMarketRankingCache domesticMarketRankingCache = new DomesticMarketRankingCache();

    @Test
    @DisplayName("[상승률 내림차순으로 상위 N개를 반환한다]")
    void getGainers_returnsTopNByChangeRateDesc() {
        // given
        domesticMarketRankingCache.update(List.of(
            ranking("005930", 1.0),
            ranking("000660", 5.0),
            ranking("035420", -3.0)));

        // when
        List<MarketRankingResponse> result = domesticMarketRankingCache.getGainers(2, null);
    ...
    void getLosers_returnsTopNByChangeRateAsc() {
        ...
        // when
        List<MarketRankingResponse> result = domesticMarketRankingCache.getLosers(2, null);
    ...
    void getGainers_limitLargerThanSize_returnsAll() {
        // given
        domesticMarketRankingCache.update(List.of(ranking("005930", 1.0)));

        // when
        List<MarketRankingResponse> result = domesticMarketRankingCache.getGainers(10, null);
    ...
    void getGainers_withStockCodesFilter_onlyRanksWithinThatSet() {
        ...
        List<MarketRankingResponse> result =
            domesticMarketRankingCache.getGainers(10, Set.of("005930", "035420"));
