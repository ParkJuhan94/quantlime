...
/**
 * 2026-09-14 이벤트화 이후 이 클래스는 백로그 재발행만 담당한다 - 실제 자막
 * 조회 로직 검증은 {@link TranscriptProcessingServiceTest} 참고.
 */
@Tag("unit")
...
class TranscriptCollectionFacadeTest {
    ...
    @Test
    @DisplayName("[후보 영상마다 VideoSelectedEvent를 재발행하고 발행 건수를 반환한다]")
    void publishBacklog_publishesEventPerCandidateAndReturnsCount() {
        // given
        Video first = videoOf(1L, "vid-1");
        Video second = videoOf(2L, "vid-2");
        given(videoRepository.findTranscribeCandidates(any(), anyInt(), any()))
            .willReturn(new SliceImpl<>(List.of(first, second)));

        // when
        int published = transcriptCollectionFacade.publishBacklog();

        // then
        assertThat(published).isEqualTo(2);
        verify(eventPublisher).publishEvent(new VideoSelectedEvent(1L));
        verify(eventPublisher).publishEvent(new VideoSelectedEvent(2L));
    }
