...
@Table(name = "domestic_regular_close_price",
    ...
    // idx_domestic_regular_close_stock_date(stock_code, trade_date DESC)는
    // uk_domestic_regular_close_stock_date와 컬럼 구성이 완전히 같아
    // 제거했다(2026-09 성능 감사). MySQL은 오름차순 인덱스를 역방향으로도
    // 스캔할 수 있어(EXPLAIN상 "Index lookup ... (reverse)") 단일 컬럼
    // 정렬만으로는 별도 DESC 인덱스가 필요 없다.
)
@Getter
