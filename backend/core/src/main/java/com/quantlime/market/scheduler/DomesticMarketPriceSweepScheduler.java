 ...
 *
 * <p>원래는 {@code DomesticWatchlistPriceRelayScheduler}(관심종목 전용, 당시 이름은
 ...
 * {@code DomesticWatchlistPriceRelayScheduler}는 Toss를 직접 호출하지 않고 여기서
...
public class DomesticMarketPriceSweepScheduler {
    ...
    // 필드명이 PriceCacheConfig의 @Bean 메서드명(domesticPreviousCloseCache)과
    ...
    private void doRefresh() {
        ...
        Map<String, Double> previousCloseByStockCode = domesticPreviousCloseCache.get(stockCodes);
        ...
        domesticMarketRankingCache.update(ranking);
    }
    ...
    private List<MarketRankingResponse> fetchChunk(
        List<String> chunkCodes, Map<String, Stock> stockByCode, Map<String, Double> previousCloseByStockCode) {
