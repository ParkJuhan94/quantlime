 ...
 * {@code DomesticMarketPriceSweepScheduler}가 토스(국내 전용) 현재가 API만 호출하므로
 ...
 *
 * <p>추가로 {@code price_unsupported=false}인 종목만 담는다 - KONEX·스팩·
 * 상폐 잔존 코드처럼 Toss가 조회 시 stock-not-found(404)만 내는 종목은
 * {@code MarketDataRefreshService}가 최초 발견 시 표시해두는데, 그런 종목이
 * prices 배치에 섞이면 배치 전체가 404로 스킵돼 랭킹이 통째로 비는 원인이
 * 되므로 스윕 유니버스에서 제외한다.
 *
 * <p>종목마스터는 {@code MarketDataRefreshService.refreshAll()}(매일 16:00
 * 평일 배치 등)이 갱신하므로, 짧은 주기로 도는 스케줄러가 틱마다 다시
 * 조회하지 않도록 {@value #REFRESH_INTERVAL_SECONDS}초 동안 재사용한다
 * (WatchlistedStockCodeCache와 동일한 단순 TTL 캐시 패턴 - 이쪽은 국내
 * 전용이라 시장별 인자를 받지 않는다).
 */
@Component
@RequiredArgsConstructor
public class DomesticListedStockCache {
