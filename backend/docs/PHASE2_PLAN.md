...
1. **종목 검색** — 종목명/코드로 검색
2. **관심 종목 CRUD** — 등록/해제/목록 (단일 사용자, 인증 없음)
...
| 관심목록 응답 | `List<WatchlistResponse>` (Slice 아님) | 단일 사용자, 페이징 불필요 |
...
## Part B — 관심 종목 CRUD (대부분 신규)

신규 feature 패키지 `com.quantlab.watchlist`. 엔티티는 `Stock.java` 패턴을 미러링.

4. `core/.../watchlist/domain/Watchlist.java`
   `@Entity @Getter @NoArgsConstructor(PROTECTED)`, `TimeBaseEntity` 상속.
   `@ManyToOne(fetch=LAZY) @JoinColumn(name="stock_id", foreignKey=@ForeignKey(NO_CONSTRAINT))`,
   `stock_id`에 unique 제약. private `@Builder` 생성자 + `validateWatchlist(Assert.notNull(stock,...))`
   + 정적 `of(Stock)`.
...
6. `core/.../watchlist/repository/WatchlistRepository.java` (`JpaRepository<Watchlist, Long>`)
   - `boolean existsByStock_StockCode(String)`
   - `Optional<Watchlist> findByStock_StockCode(String)`
   - `@Query("select w from Watchlist w join fetch w.stock order by w.createdAt desc") List<Watchlist> findAllWithStock()`
7. `core/.../watchlist/dto/response/WatchlistResponse.java`
   record(id, stockCode, stockName, marketType label, sector, createdAt).
...
9. `core/.../watchlist/service/WatchlistService.java`
   `StockMasterService` + `WatchlistRepository` 주입.
   - `addWatchlist(stockCode)`: `getStockByCode()`로 종목 검증 → `existsByStock_StockCode`
     이면 `ValidationException(ALREADY_EXISTS_WATCHLIST)` → `save(Watchlist.of(stock))`.
     TOCTOU 방어로 `DataIntegrityViolationException` catch → 동일 에러 변환(선택).
   - `removeWatchlist(stockCode)`: `findByStock_StockCode` orElseThrow
     `NotFoundException(NOT_FOUND_WATCHLIST)` → delete.
   - `getWatchlist()`: `findAllWithStock()` → 매퍼 (반드시 `@Transactional(readOnly=true)`,
     LAZY 프록시 접근이 세션 내에서 일어나도록).
10. `api/.../watchlist/controller/WatchlistController.java` (`@RequestMapping("/api/watchlist")`)
    - `GET` → `List<WatchlistResponse>` (200)
    - `POST /{stockCode}` → `201 Created` + `WatchlistResponse`
    - `DELETE /{stockCode}` → `204 No Content` (`ResponseEntity.noContent()`)
